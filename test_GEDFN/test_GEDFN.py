...
if __name__ == "__main__":
    #tf.reset_default_graph()  # deprecated. clear global graph stack and nodes
    tf.compat.v1.reset_default_graph()  # new usage
    
    ## ---- load in data
    # from preprocess import prepare_train_test_data
    
    # data_dir = "/home/wma36/gpu/data"
    # partition_df = pd.read_csv("/home/wma36/gpu/data/GeneNetworks/HomoSapients_htb_hq_adj.txt", sep=" ", index_col=0)
    # partition_features = [x.upper() for x in partition_df.index.tolist()] ## to upper


    ## result for S1 and S5
    # result_dir = "/home/wma36/gpu/sc_identifier/test_GEDFN/result_PBMC_batch1_0106"
    ## result for A and B
    #result_dir = "/home/wma36/gpu/sc_identifier/test_GEDFN/result_PBMC_batch1_AB_3layers_0106"
    ## result for control and stimulated
    result_dir = "/home/wma36/gpu/sc_identifier/test_GEDFN/result_PBMC_batch2_ctrlstim_0110"
    os.makedirs(result_dir, exist_ok=True)

    ## process S1 and S5 data
    #train_adata, test_adata = process_data_for_GEDFN.process_batch1_S1S5(data_dir, partition_df, result_dir)
    ## process batch1 A to B
    #train_adata, test_adata = process_data_for_GEDFN.process_batch1_ABC(data_dir, partition_df, result_dir,
    #        batch1="A", batch2="B")
    ## process batch2 control and stimulated
    # train_adata, test_adata = prepare_train_test_data.process_batch2_ctrl_stim(data_dir, partition_features, result_dir)


    #train_adata.write(result_dir+os.sep+"control_adata.h5ad")
    #test_adata.write(result_dir+os.sep+"stimulated_adata.h5ad")
    ...
    y_test = enc.transform(test_adata.obs[['cell.type']]).toarray()

    # subset of partition
    # par_genes = train_adata.var_names.tolist()
    # partition = partition_df.loc[par_genes, par_genes]
    # partition.to_csv(result_dir+os.sep+"sub_partition.csv")

    partition = pd.read_csv(result_dir+os.sep+"sub_partition.csv", index_col=0)
    partition = partition.to_numpy()

    from test_GEDFN.test_GEDFN import GEDFN
    ## initiate GEDFN
    gedfn = GEDFN(dims=[64], partition=partition, GE=True)

    sess = tf.Session()
    gedfn.train(x_train, y_train, tf_session=sess)

    ## prediction and evaluation
    y_pred = gedfn.predict(x_test, tf_session=sess)
    print("Predicted Y:", y_pred)

    acc = metrics.accuracy_score(y_test.argmax(1), y_pred.argmax(1))
    ARI = metrics.cluster.adjusted_rand_score(y_test.argmax(1), y_pred.argmax(1))
    macroF1 = metrics.f1_score(y_test.argmax(1), y_pred.argmax(1), average="macro")
    print("*****=====", "Testing accuracy: ", acc, " Testing ARI: ", ARI, 
            "Testing MacroF1: ", macroF1, "=====*****")

    ## analyze predicted cell types 
    pred_labels = y_pred.argmax(1)
    pred_onehot = np.zeros((pred_labels.size, pred_labels.max()+1))
    ...
    test_adata.obs.to_csv(result_dir+os.sep+"test_GEDFN_predicted_obs.csv")
    
    import matplotlib.pyplot as plt
    import scanpy.api as sc
    sc.pl.tsne(test_adata, color=['cell.type', "pred_celltypes"], size=15)
    plt.savefig(result_dir+os.sep+"test_GEDFN_prediction_result.png")

    ## extract important features
    var_important = gedfn.feature_importance(sess)
